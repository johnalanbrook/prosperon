...
var editor = {
  ...
  try_select() {
    /* nullify true if it should set selected to null if it doesn't find an object */
    var go = physics.pos_query(input.mouse.worldpos());
    return this.do_select(go);
  },

  do_select(obj) {
    /* select an object, if it is selectable given the current editor state */
    if (!obj) return;
    if (!obj._ed.selectable) return undefined;

    if (obj.master !== this.edit_level) {
      var testlevel = obj.master;
      while (testlevel && testlevel.master !== world && testlevel.master !== this.edit_level && testlevel !== testlevel.master) testlevel = testlevel.master;
      return testlevel;
    }
  ...
  mover(amt, snap) {
    return function (go) {
      go.pos = go.pos.add(amt);
    };
  },

  step_amt() {
    return input.keyboard.down("shift") ? 10 : 1;
  },

  on_grid(pos) {
    return pos.every(function (x) {
      return x % editor.grid_size === 0;
    });
  },
  ...
  snapper(dir, grid) {
    return function (go) {
      ...
      go.pos = go.pos.map(function (x) {
        return Math.snap(x, grid);
      }, this);
    };
  },
  ...
  key_move(dir) {
    ...
    if (input.keyboard.down("ctrl")) this.selectlist.forEach(this.snapper(dir.scale(1.01), editor.grid_size));
    else this.selectlist.forEach(this.mover(dir.scale(this.step_amt())));
  },
  ...
  */
  ...
  input_num_pressed(num) {
    ...
    if (num in this.camera_recalls) Object.assign(this.camera, this.camera_recalls[num]);
  },
  ...
  cleanpanels(panel) {
    this.curpanels = this.curpanels.filter(function (x) {
      return x.on;
    });
  },
  ...
  _sel_comp: undefined,
  get sel_comp() {
    return this._sel_comp;
  },
  set sel_comp(x) {
    if (this._sel_comp) player[0].uncontrol(this._sel_comp);

    this._sel_comp = x;

    if (this._sel_comp) player[0].control(this._sel_comp);
  },
  ...
  draw() {
    this.selectlist.forEach(x => {
      if ("gizmo" in x && typeof x["gizmo"] === "function") x.gizmo();
    });
  ...
  gui() {
    ...
    if (this.comp_info && this.sel_comp) render.text(input.print_pawn_kbm(this.sel_comp, false), [100, 700], 1);
    ...
    editor.color_depths = [];
    for (var i = 1; i > 0; i -= 0.1) editor.color_depths.push(colormap.sample(i));
    ...
    for (var lvl of lvlchain) {
      ...
      if (alldirty) lvl._ed.dirty = true;
      else {
        if (!lvl._ed) continue;
        lvl.check_dirty();
    ...
    if (this.rotlist.length === 1) render.text(Math.places(this.rotlist[0].angle, 3), input.mouse.screenpos(), 1, Color.teal);
    ...
    editor.edit_level.objects.forEach(function (obj) {
      if (!obj._ed.selectable) render.text("lock", obj, screenpos());
    });

    render.grid(1, editor.grid_size, editor.grid_color);
    var startgrid = game.camera.view2world([-20, 0]).map(function (x) {
      return Math.snap(x, editor.grid_size);
    });
    ...
    if (this.curpanel && this.curpanel.on) this.curpanel.gui();
...
editor.inputs.drop = function (str) {
  ...
  if (this.selectlist.length === 0) return editor.new_from_img(str);
...
editor.inputs.f9 = function () {
  os.capture("capture.bmp", 0, 0, 500, 500);
};
...
editor.inputs["C-`"] = function () {
  editor.openpanel(replpanel);
};
...
editor.inputs["C-h"] = function () {
  world.objects.forEach(function (x) {
    x.visible = true;
  });
};
editor.inputs["C-h"].doc = "Unhide all objects.";

editor.inputs["C-e"] = function () {
  editor.openpanel(assetexplorer);
};
editor.inputs["C-e"].doc = "Open asset explorer.";

editor.inputs["C-l"] = function () {
  editor.openpanel(entitylistpanel);
};
...
editor.inputs["C-m"] = function () {
  if (editor.sel_comp) {
    if ("flipy" in editor.sel_comp) editor.sel_comp.flipy = !editor.sel_comp.flipy;
...
editor.inputs.m = function () {
  if (editor.sel_comp) {
    if ("flipx" in editor.sel_comp) editor.sel_comp.flipx = !editor.sel_comp.flipx;
...
editor.inputs.q = function () {
  editor.comp_info = !editor.comp_info;
};
...
editor.inputs.f = function () {
  ...
  editor.selectlist.forEach(function (obj) {
...
editor.inputs.r.released = function () {
  editor.rotlist = [];
};
...
editor.inputs.escape = function () {
  editor.openpanel(quitpanel);
};
...
editor.inputs["C-s"] = function () {
  ...
  if (editor.selectlist.length === 0) {
    if (editor.edit_level === editor.desktop) {
      ...
      return;
    } else saveobj = editor.edit_level;
  } else if (editor.selectlist.length === 1) saveobj = editor.selectlist[0];
...
editor.inputs["C-S"] = function () {
  if (editor.selectlist.length !== 1) return;
  if (this.selectlist[0].ur !== "empty") saveaspanel.stem = this.selectlist[0].ur + ".";
  else saveaspanel.stem = "";
...
editor.inputs["C-z"] = function () {
  editor.undo();
};
editor.inputs["C-z"].doc = "Undo the last change made.";

editor.inputs["C-S-z"] = function () {
  editor.redo();
};
editor.inputs["C-S-z"].doc = "Redo the last undo.";

editor.inputs.t = function () {
  editor.selectlist.forEach(function (x) {
    x._ed.selectable = false;
  });
};
editor.inputs.t.doc = "Lock selected objects to make them non selectable.";

editor.inputs["M-t"] = function () {
  editor.edit_level.objects.forEach(function (x) {
    x._ed.selectable = true;
  });
};
...
editor.inputs["C-S-o"] = function () {
  if (!editor.edit_level._ed.dirty) editor.load_prev();
};
editor.inputs["C-S-o"].doc = "Open previous level.";

editor.inputs["C-y"] = function () {
  texteditor.on_close = function () {
...
editor.inputs["M-y"] = function () {
  editor.programmode = !editor.programmode;
};
...
editor.inputs.minus = function () {
  ...
  if (editor.working_layer > -1) editor.working_layer--;
};
...
editor.inputs.plus = function () {
  ...
  if (editor.working_layer < 4) editor.working_layer++;
};

editor.inputs.plus.doc = "Go up one working layer, or, move selected objects down one layer.";

editor.inputs["C-f1"] = function () {
  editor.edit_mode = "basic";
};
editor.inputs["C-f1"].doc = "Enter basic edit mode.";
editor.inputs["C-f2"] = function () {
  editor.edit_mode = "brush";
};
...
editor.inputs.lm = function () {
  editor.sel_start = input.mouse.worldpos();
};
...
editor.inputs.lm.released = function () {
  ...
  editor.selectlist = [];
  selects.forEach(function (x) {
    if (x !== undefined) this.selectlist.push(x);
  }, this);
};

editor.inputs.rm = function () {
  if (editor.brush_obj) editor.brush_obj = undefined;
...
editor.try_pick = function () {
  ...
  if (editor.sel_comp && "pick" in editor.sel_comp) return editor.sel_comp.pick(input.mouse.worldpos());
...
editor.inputs.mouse.move = function (pos, dpos) {
  if (editor.mousejoy) {
    if (editor.z_start) editor.camera.zoom -= dpos.y / 500;
    else if (editor.joystart) editor.camera.pos = editor.camera.pos.sub(game.camera.dir_view2world(dpos));
  }
...
editor.inputs["C-M-S-lm"] = function () {
  editor.selectlist[0].set_center(input.mouse.worldpos());
};
...
editor.inputs["C-u"] = function () {
  this.selectlist.forEach(x => x.revert());
};
...
editor.inputs["C-S-g"] = function () {
  editor.openpanel(groupsaveaspanel);
};
  ...
  if (editor.sel_comp) {
    ...
    if ("pos" in editor.sel_comp) {
      ...
      var o = {
        ...
        move(d) {
          comp.pos = comp.pos.add(comp.gameobject.dir_world2this(d));
        },
...
editor.inputs.g.released = function () {
  editor.grabselect = [];
  input.mouse.normal();
};

editor.inputs.up = function () {
  this.key_move([0, 1]);
};
editor.inputs.up.rep = true;

editor.inputs.left = function () {
  this.key_move([-1, 0]);
};
editor.inputs.left.rep = true;

editor.inputs.right = function () {
  this.key_move([1, 0]);
};
editor.inputs.right.rep = true;

editor.inputs.down = function () {
  this.key_move([0, -1]);
};
...
editor.inputs.tab = function () {
  ...
  if (!this.sel_comp) this.sel_comp = sel.nth(0);
  else {
    var idx = sel.findIndex(this.sel_comp) + 1;
    if (idx >= Object.keys(sel).length) this.sel_comp = undefined;
    else this.sel_comp = sel.nth(idx);
  }
};
...
editor.inputs["M-g"] = function () {
  if (this.sel_comp && "pick_all" in this.sel_comp) this.grabselect = this.sel_comp.pick_all();
};
...
var brushmode = {};
brushmode.inputs = {};
brushmode.inputs.lm = function () {
  editor.inputs["C-v"].call(editor);
};
...
editor.inputs.s.released = function () {
  this.scalelist = [];
};
...
var replpanel = Object.copy(inputpanel, {
  ...
  action() {
    ...
    for (var key in repl_obj.objects) ecode += `var ${key} = editor.edit_level.objects['${key}'];`;
...
replpanel.inputs.tab = function () {
  ...
  if (this.value.startsWith("this.")) keyobj = keyobj.replace("this", replobj);

  if (!this.value.includes(".")) keys.push("this");

  if (eval(`typeof ${keyobj.tofirst(".")}`) === "object" && eval(`typeof ${keyobj.replace(".", "?.")}`) === "object") obj = eval(keyobj);
  ...
  for (var k in editor.get_this()) keys.push(k);

  var comp = "";
  if (stub) comp = input.tabcomplete(stub, keys);
  else if (!this.value.includes(".")) comp = input.tabcomplete(o, keys);
  else comp = input.tabcomplete("", keys);

  if (stub) this.value = o + "." + comp;
  else if (this.value.endsWith(".")) this.value = o + "." + comp;
  else this.value = comp;
  ...
  keys = keys.map(function (x) {
    if (typeof obj[x] === "function") return esc.color(Color.Apple.orange) + x + esc.reset;
    if (Object.isObject(obj[x])) return esc.color(Color.Apple.purple) + x + esc.reset;
    if (Array.isArray(obj[x])) return esc.color(Color.Apple.green) + x + esc.reset;

    return x;
  });

  if (keys.length > 1) say(keys.join(", "));
};
...
replpanel.inputs["C-n"] = function () {
  ...
  if (this.prevmark < 0) {
    this.prevmark = -1;
    this.value = "";
  } else this.value = this.prevthis[this.prevmark];
...
replpanel.inputs.mm = function () {
  this.mm = true;
};
replpanel.inputs.mm.released = function () {
  this.mm = false;
};

replpanel.inputs.mouse.move = function (pos, dpos) {
  if (this.mm) this.scrolloffset.y -= dpos.y;
};
...
var groupsaveaspanel = Object.copy(inputpanel, {
  title: "group save as",
  action() {
    editor.groupsaveas(editor.selectlist, this.value);
  },
});
